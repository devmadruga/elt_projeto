...
def get_rj_table_info_from_url(URL="http://www.insideairbnb.com/get-the-data/"):
    """
...
def get_urls_from_rj_table_info(data="/tmp/info_from_rj_table.html"):
    ...
    if lm_listings == 'Wed, 22 Jun 2022 17:58:20 GMT' and lm_reviews == 'Wed, 22 Jun 2022 17:58:22 GMT':
        try:
...
def extract_data_from_url(ti):
    ...
    if path.exists(datafile_listings) == False and path.exists(datafile_reviews) == False:
...
with DAG(dag_id="extract_dag",
         description="DAG que extrai dados de uma url e os carrega no /tmp do container.",
...
get_rj_table_info_from_url_task >> get_urls_from_rj_table_info_task >> extract_data_from_url_task
